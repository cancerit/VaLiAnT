...
def _load_codon_table(fp: Optional[str]) -> CodonTable:
    if not fp:
...
def _load_pam_protection_vcf(sgrna_ids: Set[str], pam: Optional[str]) -> PamProtectionVariantRepository:
...
def get_oligo_templates(
    ...
    def get_missing_sgrna_err(genomic_range: GenomicRange, sgrna_ids: Set[str]) -> ValueError:
        return ValueError(
...
@click.command()
@click.argument('oligo-info', type=click.Path(exists=True))
@click.argument('ref-fasta', type=click.Path(exists=True))
@click.argument('output', type=click.Path(exists=True))
@click.argument('species')
@click.argument('assembly')
@click.option('--codon-table', type=click.Path(exists=True))
@click.option('--gff', type=click.Path(exists=True))
@click.option('--pam', type=click.Path(exists=True))
...
def main(
    ...
    for seq in (adaptor_5, adaptor_3):
        if seq and not is_dna(seq):
            logging.critical("Invalid adaptor sequence '%s'!" % seq)
            sys.exit(1)
    ...
    ref_ranges: Set[GenomicRange] = rsrs.ref_ranges
    ...
    all_mutators: Set[TargetonMutator] = rsrs.mutarors
